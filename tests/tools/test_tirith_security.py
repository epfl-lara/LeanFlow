...
class TestEnsureInstalled:
    ...
    @patch(
        "tools.implementations.tirith_security.shutil.which", return_value="/usr/local/bin/tirith"
    )
    @patch("tools.implementations.tirith_security._load_security_config")
...
class TestFailedDownloadCaching:
    ...
    @patch(
        "tools.implementations.tirith_security._install_tirith",
...
class TestExplicitPathNoAutoDownload:
    ...
    @patch("tools.implementations.tirith_security._is_install_failed_on_disk", return_value=False)
    @patch(
        "tools.implementations.tirith_security._install_tirith", return_value=("/auto/tirith", "")
    )
    @patch("tools.implementations.tirith_security.shutil.which", return_value=None)
...
class TestCosignVerification:
    ...
    @patch(
        "tools.implementations.tirith_security.subprocess.run",
    ...
    @patch(
        "tools.implementations.tirith_security.shutil.which", return_value="/usr/local/bin/cosign"
    )
    @patch("tools.implementations.tirith_security._download_file")
    @patch(
        "tools.implementations.tirith_security._detect_target", return_value="aarch64-apple-darwin"
    )
    ...
    def test_install_aborts_when_cosign_missing(self, mock_target, mock_dl, mock_which):
    ...
    def test_install_quiet_mode_downgrades_cosign_missing_log(
        self, mock_target, mock_dl, mock_which, mock_warning, mock_debug
    ):
    ...
    def test_install_aborts_when_cosign_exec_fails(
        self, mock_target, mock_dl, mock_which, mock_cosign
    ):
    ...
    def test_install_aborts_when_cosign_artifacts_missing(self, mock_target, mock_dl):
...
class TestBackgroundInstall:
    def test_ensure_installed_non_blocking(self):
        ...
        with (
            patch(
                "tools.implementations.tirith_security._load_security_config",
                return_value={
                    "tirith_enabled": True,
                    "tirith_path": "tirith",
                    "tirith_timeout": 5,
...
class TestDiskFailureMarker:
    def test_mark_and_check(self):
        ...
        with patch(
            "tools.implementations.tirith_security._failure_marker_path", return_value=marker
        ):
    ...
    def test_cosign_missing_marker_clears_when_cosign_appears(self):
        ...
        with patch(
            ...
            with patch(
                "tools.implementations.tirith_security.shutil.which",
    ...
    @patch(
        "tools.implementations.tirith_security._install_tirith",
        return_value=(None, "cosign_missing"),
    )
    @patch("tools.implementations.tirith_security.shutil.which", return_value=None)
