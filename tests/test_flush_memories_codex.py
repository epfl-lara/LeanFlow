...
class TestFlushMemoriesUsesAuxiliaryClient:
    ...
    def test_flush_uses_auxiliary_when_available(self, monkeypatch):
        ...
        with patch("agent.providers.auxiliary_client.call_llm", return_value=mock_response) as mock_call:
            messages = [
                {"role": "user", "content": "Hello"},
                {"role": "assistant", "content": "Hi there"},
                {"role": "user", "content": "Remember this"},
            ]
            with patch("tools.implementations.memory_tool.memory_tool", return_value="Saved.") as mock_memory:
...
class TestFlushMemoriesCodexFallback:
    ...
    def test_codex_mode_no_aux_uses_responses_api(self, monkeypatch):
        ...
        with patch("agent.providers.auxiliary_client.call_llm", side_effect=RuntimeError("no provider")), \
             ...
             patch("tools.implementations.memory_tool.memory_tool", return_value="Saved.") as mock_memory:
