...
from opengauss_cli.config import get_config_path, load_config
from opengauss_cli.project import discover_opengauss_project, initialize_opengauss_project
...
def test_load_config_imports_legacy_gauss_payload(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("OPENGAUSS_HOME", str(tmp_path / "open-home"))
    ...
    assert config["opengauss"]["project"]["template_source"] == "legacy-template"
    assert config["opengauss"]["workflow"]["managed_state_dir"] == ".gauss/workflow-state"
    assert config["toolsets"] == ["opengauss-native"]
    assert get_config_path().exists()


def test_initialize_project_uses_opengauss_manifest(monkeypatch, tmp_path):
    ...
    loaded = discover_opengauss_project(root)

    assert project.manifest_path == root / ".opengauss" / "project.yaml"
...
def test_discover_project_imports_legacy_manifest(monkeypatch, tmp_path):
    ...
    project = discover_opengauss_project(root)

    assert project.name == "legacy-demo"
    assert project.manifest_path == root / ".opengauss" / "project.yaml"
    assert project.runtime_dir == root / ".opengauss" / "runtime"
