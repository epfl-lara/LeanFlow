...
def test_workflow_startup_guidance_mentions_user_approved_swarm(monkeypatch):
    monkeypatch.setenv("OPENGAUSS_NATIVE_PARALLEL_AGENTS", "3")
    monkeypatch.setenv("OPENGAUSS_NATIVE_USER_APPROVED_SWARM", "1")
...
def test_history_status_lines_summarize_message_counts(monkeypatch):
    ...
    monkeypatch.setenv("OPENGAUSS_NATIVE_MODEL", "zai-org/GLM-5")
    monkeypatch.setenv("OPENGAUSS_PROJECT_ROOT", "/tmp/project")
...
def test_build_agent_uses_opengauss_native_toolset(monkeypatch):
    captured = {}
    ...
    monkeypatch.setenv("OPENGAUSS_NATIVE_API_KEY", "sk-test")
    monkeypatch.setenv("OPENGAUSS_NATIVE_PROVIDER", "zai")
    monkeypatch.setenv("OPENGAUSS_NATIVE_API_MODE", "responses")
    monkeypatch.setenv("AGENT_MAX_TURNS", "77")

    runner._build_agent()

    assert captured["enabled_toolsets"] == ["opengauss-native"]
...
def test_build_agent_uses_swarm_toolset_when_user_enabled_swarm(monkeypatch):
    ...
    monkeypatch.setenv("OPENGAUSS_NATIVE_PROVIDER", "zai")
    monkeypatch.setenv("OPENGAUSS_NATIVE_API_MODE", "responses")
    monkeypatch.setenv("OPENGAUSS_NATIVE_TOOLSET", "opengauss-native-swarm")
...
def test_write_workflow_checkpoint_persists_index_and_current(monkeypatch, tmp_path):
    monkeypatch.setenv("OPENGAUSS_HOME", str(tmp_path))
...
def test_drive_autonomous_followups_retries_until_live_state_is_verified(monkeypatch):
    monkeypatch.setenv("OPENGAUSS_NATIVE_WORKFLOW_KIND", "autoprove")
    monkeypatch.setenv("OPENGAUSS_NATIVE_AUTONOMOUS_FOLLOWUPS", "3")
    ...
    assert agent.calls[0]["persist_user_message"] == "[opengauss-native autonomous continuation #1]"
