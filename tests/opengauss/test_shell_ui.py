...
from opengauss_cli.main import InteractiveShell
from opengauss_cli.workflow_state import append_workflow_run_log, reset_workflow_run_log
...
def test_prompt_message_includes_project_and_phase(monkeypatch, tmp_path):
    ...
    (root / "lakefile.lean").write_text("import Lake\nopen Lake DSL\npackage demo\n", encoding="utf-8")
    (root / "lean-toolchain").write_text("leanprover/lean4:v4.20.0\n", encoding="utf-8")
    (root / ".opengauss").mkdir()
    (root / ".opengauss" / "project.yaml").write_text(
        "schema_version: 1\nname: Demo\nkind: lean4\nlean_root: .\ncreated_at: now\npaths:\n  runtime: .opengauss/runtime\n  cache: .opengauss/cache\n  workflows: .opengauss/workflows\nsource:\n  mode: init\n  template_source: ''\nblueprint:\n  markers: []\n",
        encoding="utf-8",
    )
    (root / ".opengauss" / "runtime").mkdir()
    (root / ".opengauss" / "cache").mkdir()
    (root / ".opengauss" / "workflows").mkdir()
