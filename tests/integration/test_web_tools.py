...
class WebToolsTester:
    ...
    def test_web_search(self) -> list[str]:
        ...
        for query, limit in test_queries:
            try:
                ...
                if "error" in data:
                    self.log_result(
                        f"Search: {query[:30]}...", "failed", f"API error: {data['error']}"
                    )
                    continue
                ...
                if "success" not in data or "data" not in data:
                    self.log_result(
                        f"Search: {query[:30]}...", "failed", "Missing success or data fields"
                    )
                    if self.verbose:
                ...
                if valid_results == len(web_results):
                    self.log_result(
                        f"Search: {query[:30]}...", "passed", f"All {valid_results} results valid"
                    )
                else:
                    self.log_result(
                        f"Search: {query[:30]}...",
                ...
                self.log_result(
                    f"Search: {query[:30]}...", "failed", f"Exception: {type(e).__name__}: {str(e)}"
                )
                if self.verbose:
                    import traceback
    ...
    async def test_web_extract(self, urls: list[str] = None):
        ...
        if not urls:
            urls = ["https://docs.firecrawl.dev/introduction", "https://www.python.org/about/"]
        ...
        if urls:
            try:
                ...
                self.log_result(
                    "Extract (no LLM)", "failed", f"Exception: {type(e).__name__}: {str(e)}"
                )
                if self.verbose:
                    import traceback
    ...
    async def test_web_extract_with_llm(self, urls: list[str] = None):
        ...
        try:
            ...
            if content:
                ...
                if content_len > 0:
                    self.log_result(
                        "Extract (with LLM)", "passed", f"Content processed: {content_len} chars"
                    )
    ...
    async def test_web_crawl(self):
        ...
        for url, instructions, expected_min_pages in test_sites:
            try:
                ...
                self.log_result(
                    f"Crawl: {url}", "failed", f"Exception: {type(e).__name__}: {str(e)}"
                )
                if self.verbose:
                    import traceback
