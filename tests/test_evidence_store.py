...
script_path = (
    repo_root / "optional-skills" / "security" / "oss-forensics" / "scripts" / "evidence-store.py"
)
...
def test_evidence_store_query_searches_multiple_fields(tmp_path):
    ...
    store.add(
        source="git_fsck", content="dangling commit abc123", evidence_type="git", actor="attacker"
    )
