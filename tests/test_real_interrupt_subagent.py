...
class TestRealSubagentInterrupt(unittest.TestCase):
    ...
    def test_interrupt_child_during_api_call(self):
        ...
        def run_delegate():
            try:
                ...
                with patch("run_agent.OpenAI") as MockOpenAI:
                    ...
                    with patch.object(
                        AIAgent, "_build_system_prompt", return_value="You are a test agent"
                    ):
        ...
        if child:
            ...
            self.assertTrue(child._interrupt_requested, "Interrupt did not propagate to child!")
        ...
        self.assertLess(
            elapsed, 3.0, f"Took {elapsed:.2f}s — interrupt was not detected quickly enough"
        )
        self.assertEqual(
            result["status"], "interrupted", f"Expected 'interrupted', got '{result['status']}'"
        )
