"""Runtime provider resolution for the EPFLemma kernel."""
...
def resolve_requested_provider(requested: str | None = None) -> str:
    ...
    env_provider = os.getenv("EPFLEMMA_INFERENCE_PROVIDER", "").strip()
    if env_provider:
...
def _resolve_local_runtime() -> dict[str, Any]:
    ...
    if not runtime:
        raise RuntimeProviderError(
            "No active local runtime configured. Use `epflemma models local use <runtime> <model>` first."
        )
    return {
        "provider": "local",
        "api_mode": "chat_completions",
