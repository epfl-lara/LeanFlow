...
def _read_codex_tokens(*, allow_legacy_store: bool | None = None) -> dict[str, str]:
    """Read Codex OAuth tokens from EPFLemma auth, optionally falling back to Codex CLI auth.

    EPFLemma-owned auth remains the default to avoid surprising provider
    ...
    auth_data = _read_json_file(get_epflemma_home() / "auth.json")
    ...
    if allow_legacy_store is None:
        allow_legacy_store = _truthy_env("EPFLEMMA_USE_LEGACY_CODEX_AUTH")
