"""Curated EPFLemma skill discovery and loading."""
...
PROJECT_SKILL_DIRNAME = ".epflemma"
...
def _repo_root() -> Path:
    return Path(__file__).resolve().parent.parent.parent  # epflemma_cli/runtime/X.py -> repo root


def _builtin_root() -> Path:
    return _repo_root() / "epflemma_skills"
...
def _user_root() -> Path:
    return _epflemma_home() / "skills"


def _project_root(cwd: str | os.PathLike[str] | None = None) -> Path:
    base = Path(cwd or os.getcwd()).expanduser().resolve()
    for candidate in (base, *base.parents):
        epflemma_dir = candidate / PROJECT_SKILL_DIRNAME
        if epflemma_dir.is_dir():
            return epflemma_dir / "skills"
        for dirname in LEGACY_PROJECT_DIRNAMES:
            legacy_dir = candidate / dirname
            if legacy_dir.is_dir():
                return legacy_dir / "skills"
...
def build_skill_prompt(name: str, cwd: str | os.PathLike[str] | None = None) -> str:
    ...
    if not payload:
        return ""
    parts = [
        f"[EPFLEMMA SKILL: {payload['name']} ({payload['source']})]",
        "",
