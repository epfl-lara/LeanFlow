"""Interactive REPL shell for the `epflemma` CLI.
...
historical ``from epflemma_cli.main import InteractiveShell`` surface. Tests that drive shell
methods patch collaborators on ``epflemma_cli.shell``; tests driving ``main()`` patch them on
``epflemma_cli.main`` — both modules import the same names independently.
"""
