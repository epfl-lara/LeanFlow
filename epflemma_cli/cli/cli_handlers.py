"""Argparse subcommand handlers and JSON/print helpers for the epflemma CLI.

Phase 3 extraction: these top-level handler/formatter functions were lifted verbatim out of
``epflemma_cli.main`` to shrink the CLI entrypoint module. They are the cohesive,
...
Handlers that reference names patched in tests via ``epflemma_cli.main.<name>`` (e.g.
...
``main`` re-imports every name defined here (``from epflemma_cli.cli.cli_handlers import ...``) so
