...
\newcommand{\method}{\textsc{LeanFlow}\xspace}
...
\icmltitle{\method: A Case Study in Workflow-Driven Lean Autoformalization}
  ...
  \icmlkeywords{autoformalization, theorem proving, Lean, mathematical reasoning, agents}
...
\begin{abstract}
  We present and evaluate \method, a \lean-specialized LLM-agent system for translating mathematical papers into buildable \lean projects.
  Recent verifier-in-the-loop systems show that large formal artifacts can be produced, but it remains unclear which runtime mechanisms make document-to-project formalization reliable across model strengths.
  We study this question on two previously unformalized mathematical papers in number theory and measure theory, using model ablations (\kimi and \gpt) and proof-workflow and toolset ablations; we report task outcome, API calls, input tokens, and output tokens.
  The full workflow completes both document-level projects with \kimi while no-queue variants hit the 2000-call budget; with \gpt, it lowers model-side token use on both sources.
  On complementary benchmarks, \method reaches 75.7\% BEq+ on RLM25, 97\% on FATE-H, and solves all five ICML 2026 AI for Math TCS challenge projects in our \gpt runs.
\end{abstract}
...
This separation is useful across the two model settings in our experiments.
For \kimi~\citep{moonshot2026kimi}, it turns a long, failure-prone transcript into bounded theorem assignments with explicit context and failure memory.
For \gpt, the same structure reduces waste: the model can spend more of its budget on local proof search instead of rediscovering the active theorem, rechecking the whole project, or deciding for itself whether a run is finished.
...
Second, it evaluates those design choices through ablations under \kimi and \gpt on document-level case studies, with complementary measurements on RLM25, FATE-H, and the AI for Math TCS challenge.
...
Our scope is smaller in document volume but different in emphasis: we isolate queue control, tool routing, statement handoff, and model-side budget under the two model settings in our experiments.
...
\cref{tab:longform-ablation} shows that queue management changes the outcome for \kimi on these document-level sources: both no-queue variants exhaust the 2000-call budget on both case studies, while full \method succeeds in 1043 calls on Pythagorean triples and 1278 calls on Cramer--Wold.
...
For \gpt, all document-level variants succeed, so the effect is mostly budget use rather than binary success; full \method has the lowest or tied-lowest input-token cost on both sources, although the Cramer--Wold no-queue/full-tools run uses slightly fewer calls at higher input-token cost.
...
First, project buildability is necessary but not sufficient: a project can build with a faithful theorem, a weakened theorem, or a theorem whose representation silently changed.
We mitigate this through the statement/source gate, but the document-level skeletons still rely on expert judgment before proof ablations begin; source spans and blueprint decisions should therefore be released with the artifacts so disagreements are inspectable.
Second, two document-level case studies cannot establish broad coverage across mathematics, and the Cramer--Wold skeleton was initialized with \gpt rather than \kimi because of its representation choices.
Third, model comparisons are sensitive to model version, serving stack, context limit, retry policy, and tool latency.
Calls and token counts are useful proxies for model-side effort, but they are not a full measure of wall-clock cost or human engineering cost.
...
The current results show that this structure lets \kimi finish document-level sources that the no-queue variants do not finish within budget, helps \gpt spend less model budget, and separates statement synthesis, proof-only proving, and algorithmic proof workloads.

\section*{Impact Statement}

This paper presents work whose goal is to make \lean autoformalization more reliable, inspectable, and reproducible.
