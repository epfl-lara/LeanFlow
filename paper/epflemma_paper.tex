...
\begin{abstract}
  ...
  Recent verifier-in-the-loop systems show that large formal artifacts can be produced, but it remains unclear which runtime mechanisms improve document-to-project formalization across model strengths.
  We study this question on two previously unformalized mathematical papers in number theory and measure theory, using model, proof-workflow, and toolset ablations with \kimi and \gpt; we report task outcome, API calls, input tokens, and output tokens.
  The full workflow completes both document-level projects with \kimi within the 2000-call budget, while no-queue variants hit the budget; with \gpt, it lowers model-side token use on both sources.
