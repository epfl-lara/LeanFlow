"""Curated OpenGauss skill discovery and loading."""
...
def _builtin_root() -> Path:
    return _repo_root() / "opengauss_skills"
...
def _user_root() -> Path:
    return _opengauss_home() / "skills"


def _project_root(cwd: str | os.PathLike[str] | None = None) -> Path:
    base = Path(cwd or os.getcwd()).expanduser().resolve()
    for candidate in (base, *base.parents):
        opengauss_dir = candidate / ".opengauss"
        if opengauss_dir.is_dir():
            return opengauss_dir / "skills"
    return base / ".opengauss" / "skills"
...
def build_skill_prompt(name: str, cwd: str | os.PathLike[str] | None = None) -> str:
    ...
    if not payload:
        return ""
    parts = [
        f"[OPENGAUSS SKILL: {payload['name']} ({payload['source']})]",
        "",
