"""OpenGauss config and environment management."""
...
OPENGAUSS_HOME_ENV = "OPENGAUSS_HOME"
LEGACY_HOME_ENV = "GAUSS_HOME"
LEGACY_HOME_DEFAULT = Path.home() / ".gauss"
OPENGAUSS_HOME_DEFAULT = Path.home() / ".opengauss"
...
def get_opengauss_home() -> Path:
    explicit = os.getenv(OPENGAUSS_HOME_ENV, "").strip()
    if explicit:
        return Path(explicit).expanduser()
    legacy = os.getenv(LEGACY_HOME_ENV, "").strip()
    if legacy and Path(legacy).expanduser().name == ".opengauss":
        return Path(legacy).expanduser()
    return OPENGAUSS_HOME_DEFAULT


def get_legacy_home() -> Path:
    return Path(os.getenv(LEGACY_HOME_ENV, str(LEGACY_HOME_DEFAULT))).expanduser()


def get_config_path() -> Path:
    return get_opengauss_home() / "config.yaml"


def get_env_path() -> Path:
    return get_opengauss_home() / ".env"


def get_install_root_path() -> Path:
    return get_opengauss_home() / "install-root"
...
def _transform_legacy_config(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(legacy_root, Mapping):
        merged["opengauss"]["project"]["template_source"] = str(
        ...
        merged["opengauss"]["workflow"]["managed_state_dir"] = workflow_state_dir
    ...
    if isinstance(toolsets, list):
        ...
        for name in toolsets:
            text = str(name).strip()
            if text == "gauss-native":
                text = "opengauss-native"
            elif text == "gauss-cli":
                text = "opengauss-cli"
            elif text.startswith("gauss-"):
                continue
            if text and text not in rewritten:
                rewritten.append(text)
        merged["toolsets"] = rewritten or ["opengauss-cli"]

    return merged


def _import_legacy_home(home: Path) -> None:
    legacy_home = get_legacy_home()
...
def ensure_opengauss_home(import_legacy: bool = True) -> Path:
...
def load_config() -> dict[str, Any]:
    ...
    if not isinstance(payload, Mapping):
        return deepcopy(DEFAULT_CONFIG)
    if "gauss" in payload and "opengauss" not in payload:
...
def save_config(config: Mapping[str, Any]) -> None:
    ensure_opengauss_home(import_legacy=False)
    path = get_config_path()
