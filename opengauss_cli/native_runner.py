#!/usr/bin/env python3
"""Local managed Lean workflow runner for the opengauss-native backend."""
...
MANAGED_SNAPSHOT_PREFIX = (
    "[OPENGAUSS-NATIVE MANAGED SNAPSHOT] Earlier managed workflow turns were compacted "
...
WORKFLOW_CHECKPOINT_PREFIX = (
    "[OPENGAUSS-NATIVE WORKFLOW CHECKPOINT] This persisted workflow handoff captures a "
...
LIVE_PROOF_STATE_PREFIX = (
    "[OPENGAUSS-NATIVE LIVE PROOF STATE] This is the latest runner-refreshed Lean state "
...
def _read_native_env(name: str, default: str = "") -> str:
    return _read_text_env(f"OPENGAUSS_NATIVE_{name}", _read_text_env(f"GAUSS_NATIVE_{name}", default))
...
def _managed_home() -> Path:
    return Path(
        _read_text_env(
            "OPENGAUSS_HOME",
            _read_text_env("GAUSS_HOME", str(Path.home() / ".opengauss")),
        )
    ).expanduser()


def _project_root() -> str:
    return _read_text_env("OPENGAUSS_PROJECT_ROOT", _read_text_env("GAUSS_PROJECT_ROOT", os.getcwd()))
...
def _print_runner_help() -> None:
    print("opengauss-native session commands:")
...
def _run_explicit_verification_build() -> tuple[bool, str]:
    try:
    ...
    return False, f"build reported errors: {detail[:280]}"
    ...
    for message in reversed(messages):
        ...
        if (
            ...
            updated["content"] = (
                ...
                + "\n\n[opengauss-native pruned older tool output to preserve context budget]"
            )
            pruned_messages += 1
...
def _build_agent() -> AIAgent:
    ...
    if not model:
        raise SystemExit("opengauss-native: OPENGAUSS_NATIVE_MODEL is not configured")
    if not base_url or not api_key:
        raise SystemExit("opengauss-native: provider credentials are incomplete")

    toolset_name = _read_native_env("TOOLSET", "opengauss-native") or "opengauss-native"
    agent = AIAgent(
        model=model,
        base_url=base_url,
        api_key=api_key,
    ...
    if owner_id:
        os.environ["OPENGAUSS_NATIVE_RUNNER_OWNER"] = owner_id
    return agent
...
def _managed_system_prompt() -> str:
    context_path = _read_text_env("OPENGAUSS_WORKFLOW_CONTEXT", _read_text_env("GAUSS_AUTOFORMALIZE_CONTEXT"))
    context_text = ""
    if context_path:
        try:
    ...
    sections = [
        "You are the opengauss-native managed Lean workflow backend.",
...
def _drive_autonomous_followups(
    ...
    for cycle in range(1, _autonomous_followup_limit() + 1):
        ...
        result = agent.run_conversation(
            ...
            persist_user_message=f"[opengauss-native autonomous continuation #{cycle}]",
        )
...
def main() -> int:
    ...
    try:
        ...
        result = agent.run_conversation(
            ...
            system_message=system_prompt,
            conversation_history=history,
            persist_user_message="[opengauss-native startup workflow request]",
        )
        previous_history = history[:]
        history = result["messages"]
        checkpoint_state = _journal_status()
