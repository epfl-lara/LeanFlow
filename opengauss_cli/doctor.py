...
from opengauss_cli.branding import get_cli_command_name, get_product_name
from opengauss_cli.config import ensure_opengauss_home, get_opengauss_home, load_config
from opengauss_cli.project import ProjectNotFoundError, discover_opengauss_project
from opengauss_cli.runtime_provider import format_runtime_provider_error, resolve_runtime_provider
...
def run_doctor(active_cwd: str | Path | None = None) -> tuple[list[str], str]:
    ...
    lines = [
        ...
        f"Home: {get_opengauss_home()}",
        f"Config: {get_opengauss_home() / 'config.yaml'}",
    ]
