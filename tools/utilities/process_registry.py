...
CHECKPOINT_PATH = get_leanflow_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        log_path = f"/tmp/leanflow_bg_{session.id}.log"
        pid_path = f"/tmp/leanflow_bg_{session.id}.pid"
    ...
    def wait(self, session_id: str, timeout: int = None) -> dict:
        ...
        Returns:
            ...
            and output snapshot.
        """
        from tools.implementations.terminal_tool import _interrupt_event
...
def _handle_process(args, **kw):
    ...
    return error(
        f"Unknown process action: {action}. Use: list, poll, log, wait, kill, write, submit"
    )
