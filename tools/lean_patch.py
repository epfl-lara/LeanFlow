...
``tools.lean_tool`` re-exports ``apply_verified_patch_tool`` so callers and the
tool registry keep resolving it as ``lean_tool.apply_verified_patch_tool``. This
module must NOT import ``tools.lean_tool`` (it would create an import cycle); it
reaches its collaborators directly via ``epflemma_cli.file_locks`` /
``epflemma_cli.lean_services`` / ``epflemma_cli.workflow_state`` /
``tools.file_operations`` / ``tools.patch_parser``.
"""
...
from epflemma_cli.lean_services import lean_verify
