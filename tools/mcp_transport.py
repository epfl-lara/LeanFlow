...
This module is a leaf: it does NOT import :mod:`tools.mcp_tool`, so the
re-export shim in ``mcp_tool`` introduces no import cycle. Every name here
is re-exported from ``tools.mcp_tool`` for backwards compatibility, so
callers and tests that resolve ``tools.mcp_tool.<name>`` keep working.
"""
