...
class ToolExecutor:
    ...
    def execute(
        ...
        if (len(tool_calls) <= 1
                or any(tc.function.name in run_agent._NEVER_PARALLEL_TOOLS for tc in tool_calls)):
    ...
    def execute_concurrent(
        ...
        for tool_call in tool_calls:
            ...
            if function_name in ("write_file", "patch", "apply_verified_patch") and agent._checkpoint_mgr.enabled:
                try:
        ...
        if agent.quiet_mode:
            ...
            spinner = run_agent.KawaiiSpinner(f"{face} ⚡ running {num_tools} tools concurrently", spinner_type='dots')
            spinner.start()
        ...
        for i, (tc, name, args) in enumerate(parsed_calls):
            ...
            if agent.quiet_mode:
                cute_msg = run_agent._get_cute_tool_message_impl(name, args, tool_duration, result=function_result)
                print(f"  {cute_msg}")
    ...
    def execute_sequential(
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            if preflight_result is not None:
                ...
                tool_duration = time.time() - tool_start_time
                if agent.quiet_mode:
                    agent._vprint(f"  {run_agent._get_cute_tool_message_impl('todo', function_args, tool_duration, result=function_result)}")
            elif function_name == "session_search":
                if not agent._session_db:
                ...
                tool_duration = time.time() - tool_start_time
                if agent.quiet_mode:
                    agent._vprint(f"  {run_agent._get_cute_tool_message_impl('session_search', function_args, tool_duration, result=function_result)}")
            elif function_name == "memory":
                target = function_args.get("target", "memory")
                from tools.memory_tool import memory_tool as _memory_tool
                ...
                tool_duration = time.time() - tool_start_time
                if agent.quiet_mode:
                    agent._vprint(f"  {run_agent._get_cute_tool_message_impl('memory', function_args, tool_duration, result=function_result)}")
            elif function_name == "clarify":
                from tools.clarify_tool import clarify_tool as _clarify_tool
                ...
                tool_duration = time.time() - tool_start_time
                if agent.quiet_mode:
                    agent._vprint(f"  {run_agent._get_cute_tool_message_impl('clarify', function_args, tool_duration, result=function_result)}")
            elif function_name == "delegate_task":
                from tools.delegate_tool import delegate_task as _delegate_task
                ...
                if agent.quiet_mode:
                    face = random.choice(run_agent.KawaiiSpinner.KAWAII_WAITING)
                    spinner = run_agent.KawaiiSpinner(f"{face} {spinner_label}", spinner_type='dots')
                    spinner.start()
                ...
                try:
                    ...
                    tool_duration = time.time() - tool_start_time
                    cute_msg = run_agent._get_cute_tool_message_impl('delegate_task', function_args, tool_duration, result=_delegate_result)
                    if spinner:
                ...
                preview = run_agent._build_tool_preview(function_name, function_args) or function_name
                if len(preview) > 30:
                    preview = preview[:27] + "..."
                ...
                try:
                    ...
                    tool_duration = time.time() - tool_start_time
                    cute_msg = run_agent._get_cute_tool_message_impl(function_name, function_args, tool_duration, result=_spinner_result)
                    spinner.stop(cute_msg)
            else:
                try:
                    function_result = run_agent.handle_function_call(
